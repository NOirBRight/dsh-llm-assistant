...
//#region lib/types/host-modules.js
...
*/
function createHostModuleLoader(entry = process.argv[1]) {
	...
	return async (id) => {
		...
		for (const root of roots) try {
			return await import(__rewriteRelativeImportExtension$1(pathToFileURL(createRequire(root).resolve(id)).href));
		} catch (error) {
			last = error;
		}
		throw last instanceof Error ? last : /* @__PURE__ */ new Error(`cannot resolve ${id}`);
	};
}
//#endregion
//#region lib/types/session-reference-boot.js
/**
...
*/
const SESSION_REFERENCE_BUDGET = {
	maxReferences: 3,
	maxReferenceBytes: 65536
};
async function ensureSessionReference(ctx, load) {
	if (ctx.get("sessionReferenceResolver") !== void 0) return "present";
	const plugin = sessionReferencePluginOf(await load("@deepseek-ai/dsh-session-reference"));
	if (plugin === void 0) throw new Error("@deepseek-ai/dsh-session-reference exported neither a plugin nor a default");
...
function createAssistantPort(agent, api, revision, chrome = () => ({}), attachments, taskReferenceAvailable = false) {
	return {
		snapshot() {
			...
			const status = projected.turnStartTime !== void 0 || projected.pending !== "" || projected.thinking !== "" ? "running" : "idle";
			const extra = chrome();
			return {
				sessionId: agent.id,
...
const DENY_DUTY_SCHEDULE = [
	"schedule_create",
	"schedule_delete",
	"schedule_list"
];
const DENY_DUTY_TOOLS = [...DENY_ASSISTANT_TOOLS, ...DENY_DUTY_SCHEDULE];
...
const HEARTBEAT_SCHEDULE_ID = "heartbeat";
const DUTY_MAX_EVENT_BYTES = 65536;
...
function foldEverySchedules(events) {
...
function hasHeartbeatSchedule(events) {
	...
	return record !== void 0 && record.everySeconds === 1800 && foldEverySchedules(events).size === 1;
}
function staleHeartbeatIds(events) {
	return [...foldEverySchedules(events).values()].filter((record) => record.id !== "heartbeat" || record.everySeconds !== 1800).map((record) => record.id);
}
...
function countDutyTurns(events) {
	let turns = 0;
	for (const event of events) if (event.type === "turn/start") turns += 1;
	return turns;
}
function dutySessionIsOversized(events) {
	if (countDutyTurns(events) > 20) return true;
	if (latestDutySeq(events) > 80) return true;
	try {
		return JSON.stringify(events).length > DUTY_MAX_EVENT_BYTES;
	} catch {
		return events.length > 80;
	}
}
function shouldReplaceDutySession(events) {
...
function shouldRotateDutyAfterQuiet(events) {
	const text = lastAssistantText(events);
	if (text === void 0 || !text.startsWith("HEARTBEAT_QUIET")) return false;
	return countDutyTurns(events) >= 2 || dutySessionIsOversized(events);
}
function isDutyRelayEvent(type) {
	return type === "turn/end";
}
function dutyRelayDecision(events, cursor, eventType) {
	if (!isDutyRelayEvent(eventType)) return void 0;
...
function bootDutyRelayCursor(events, persisted) {
...
async function run(ctx) {
	...
	try {
		if (loadHost !== void 0) {
			if (await ensureSessionReference(ctx, loadHost) === "mounted") log("session-reference mounted (web-app did not provide it)");
		}
	} catch (error) {
	...
	let assistantPreset;
	try {
		if (loadHost === void 0) log("WARN process.argv[1] is missing — private assistant preset will not mount");
		else assistantPreset = createAssistantPreset({
			host: ctx,
			load: loadHost,
			log
		});
	...
	let dutyHandle;
	let duty;
	const setupDuty = (agentCtx) => {
		restrictDutyTools(agentCtx);
		registerDutyBrief(agentCtx, assistantOf);
	};
	const persistDuty = (next, extra = {}) => {
		persisted = {
			...persisted,
			dutySessionId: next.id,
			...extra
		};
		writeState(file, persisted);
	};
	const replaceDuty = async (reason, previous) => {
		...
		if (previous !== void 0) previous.dispose().catch((error) => log("WARN old duty dispose failed: " + errMsg(error)));
	};
	try {
		...
		if (duty !== void 0) {
			...
			ctx.on.bind(ctx)("session/event", (session, event) => {
				...
				else if (shouldRotateDutyAfterQuiet(duty.session.events)) replaceDuty("quiet-rotate", dutyHandle).then(() => {
					if (duty !== void 0) installHeartbeatSchedule(duty);
