...
function resultCallIdOf(data) {
	...
	if (typeof data.callId === "string") return data.callId;
	if (typeof data.id === "string") return data.id;
	const message = isObject(data.message) ? data.message : void 0;
	const source = message !== void 0 && isObject(message.source) ? message.source : void 0;
	if (typeof source?.callId === "string") return source.callId;
	const result = Array.isArray(message?.content) ? message.content.find((block) => isObject(block) && block.type === "tool-result") : void 0;
...
async function handleAssistantRpc(port, endpoint, payload, extras = {}) {
	...
	if (endpoint === "assistant/send") {
		...
		return {
			ok: true,
			value: await port.send(request.text, request.images)
		};
	}
...
function createTaskReferenceToolDefinition(deps) {
	return {
		...
		async execute(args, exec) {
			...
			let anchor = requested === void 0 ? deps.currentTask() : void 0;
			if (requested !== void 0) {
