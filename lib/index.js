...
function decodeSendRequest(payload) {
	...
	if (payload.task !== void 0 && task === void 0) return void 0;
	if (payload.refreshTaskContext !== void 0 && typeof payload.refreshTaskContext !== "boolean") return void 0;
	return {
		text,
...
function decodeTaskAnchor(value) {
	if (!isRecord$1(value) || typeof value.sessionId !== "string" || value.sessionId.trim() === "") return void 0;
...
function createAssistantPort(agent, api, revision, chrome = () => ({}), attachments, taskReferences) {
	return {
		snapshot() {
			...
			return {
				...
				...projected.context !== void 0 ? { context: projected.context } : {},
				...projected.todos.length > 0 ? { todos: projected.todos } : {},
				...projected.goal !== void 0 ? { goal: projected.goal } : {},
				taskReferenceAvailable: taskReferences !== void 0
			};
		},
		async send(text, images, task) {
			...
			try {
				...
				let receipt;
				if (task !== void 0) {
					if (taskReferences === void 0) return {
						sent: false,
						error: "任务引用功能不可用"
					};
					const previous = latestTaskReceipt(agent.session.events);
					if (task.refresh !== true && previous !== void 0 && (previous.taskId === task.anchor.sessionId || previous.sourceSessionIds.includes(task.anchor.sessionId))) receipt = previous;
					else {
						const prepared = await taskReferences.prepare({
							agent,
							content,
							anchorSessionId: task.anchor.sessionId
						});
						...
						agent.inject(api.createUserMessage({
							content: [{
								type: "text",
								text: JSON.stringify(prepared.receipt)
							}],
							source: {
								kind: "plugin",
								plugin: TASK_REFERENCE_PLUGIN
							}
						}));
						if (prepared.additionalContext !== void 0) agent.inject(prepared.additionalContext);
					}
				}
...
function latestTaskReceipt(events) {
	for (let index = events.length - 1; index >= 0; index -= 1) {
		...
		if (event?.type === "user/message") {
			...
			if (receipt !== void 0) return receipt;
		}
	}
}
...
function taskReceiptOf(value) {
	if (!isRecord(value) || typeof value.taskId !== "string" || typeof value.label !== "string") return void 0;
...
function summarizeArgs(value) {
	if (typeof value === "string") try {
		...
		return clip$1(value, 80);
	}
	if (!isObject(value)) return "";
	const command = value.command;
	if (typeof command === "string") return clip$1(command, 80);
	const path = value.path ?? value.file_path ?? value.filePath;
	if (typeof path === "string") return clip$1(path, 80);
	const title = value.title ?? value.name ?? value.query;
	if (typeof title === "string") return clip$1(title, 80);
	const keys = Object.keys(value);
	return keys.length === 0 ? "" : clip$1(keys.join(", "), 80);
}
...
function parseArgs(raw) {
	...
	return isObject(raw) ? raw : void 0;
}
...
async function handleAssistantRpc(port, endpoint, payload, extras = {}) {
	...
	if (endpoint === "assistant/send") {
		...
		return {
			ok: true,
			value: await port.send(request.text, request.images, request.task === void 0 ? void 0 : {
				anchor: request.task,
				...request.refreshTaskContext === true ? { refresh: true } : {}
			})
		};
	}
	...
	if (endpoint === "assistant/rollover") {
		if (!isRecord$1(payload) || Object.keys(payload).length !== 0) return fail("bad-request", "invalid assistant/rollover request");
...
const HEARTBEAT_SETUP_DONE = "HEARTBEAT_SETUP_DONE";
...
function hasHeartbeatSchedule(events) {
	return heartbeatEverySeconds(events) === 300;
}
...
async function run(ctx) {
	...
	try {
		...
		if (!hasHeartbeatSchedule(duty.session.events)) {
			duty.followup(runtime.createUserMessage({
				content: [{
					type: "text",
					text: `先 schedule_list。删掉所有 prompt 含 HEARTBEAT 且 every_seconds 不是 ${String(300)} 的记录，再 schedule_create：every_seconds=${String(300)}，prompt 必须一字不改：\n${HEARTBEAT_PROMPT}\n若已有 every_seconds=${String(300)} 的 HEARTBEAT 则不要再建。完成后只回复 ${HEARTBEAT_SETUP_DONE}。`
				}],
				source: { kind: "user" }
			}));
		...
		ctx.on.bind(ctx)("session/event", (session) => {
			...
			persisted = {
				...persisted,
				dutyRelayedSeq: latestDutySeq(duty.session.events)
			};
			writeState(file, persisted);
			try {
	...
	const taskReferences = sessionQuery === void 0 || sessionReferenceResolver === void 0 ? void 0 : createTaskReferenceAdapter({
	...
	const chrome = () => {
		...
		const efforts = modelGroups.flatMap((group) => group.models).find((entry) => entry.provider === selected.provider && entry.id === selected.model)?.efforts ?? [];
	...
	const livePort = {
		...
		send: (text, images, task) => rollover?.switching === true ? Promise.resolve({
			sent: false,
			error: "助理正在切换到新对话"
	...
	rollover = createSessionRollover({
		...
		commit(next) {
			...
			queueMicrotask(() => {
				try {
					hideId(nextId);
					subscribeAssistantEvents(ctx, nextId, revision, () => {
						hideId(nextId);
					});
	...
	registerAssistantRpc(ctx, livePort, {
		...
		async setModel(model, effort, provider) {
			...
			revision.current += 1;
			if (defaultModel.saveSelection !== void 0) try {
	...
	subscribeAssistantEvents(ctx, initialAssistantId, revision, () => {
		hideId(initialAssistantId);
	});
