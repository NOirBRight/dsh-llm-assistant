...
import { createHostModuleLoader } from "./host-modules.js";
import { ensureSessionReference } from "./session-reference-boot.js";
...
import { registerAssistantSse } from "./assistant-sse.js";
...
import { createAssistantPreset, ASSISTANT_PRESET_EXPECTED_TOOLS } from "./assistant-preset.js";
...
import { ASSISTANT_SAFETY_SECTION } from "./assistant-prompt.js";
import { DUTY_PLUGIN, bootDutyRelayCursor, briefJson, dutyCwd, dutyRelayDecision, installHeartbeatSchedule, shouldReplaceDutySession, shouldRotateDutyAfterQuiet, } from "./duty.js";
...
async function run(ctx) {
    ...
    try {
        if (loadHost !== undefined) {
            ...
            if (status === 'mounted')
                log('session-reference mounted (web-app did not provide it)');
        }
    }
    catch (error) {
    ...
    const replaceDuty = async (reason, previous) => {
        ...
        if (previous !== undefined)
            void previous.dispose().catch((error) => log('WARN old duty dispose failed: ' + errMsg(error)));
    };
    try {
        ...
        if (duty !== undefined) {
            ...
            if (persisted.dutyRelayedSeq !== bootCursor)
                persistDuty(duty, { dutyRelayedSeq: bootCursor });
            ...
            onDuty('session/event', (session, event) => {
                ...
                if (decision === undefined)
                    return;
                if (decision.cursor !== persisted.dutyRelayedSeq)
                    persistDuty(duty, { dutyRelayedSeq: decision.cursor });
