...
export function applyAssistantLiveDelta(snapshot, delta, seq, revision) {
    if (delta.kind === 'text')
        return { ...snapshot, seq, revision, pending: (snapshot.pending ?? '') + delta.text };
    if (delta.kind === 'reasoning')
        return { ...snapshot, seq, revision, thinking: (snapshot.thinking ?? '') + delta.text };
...
const OPTIONAL_KEYS = ['pending', 'thinking', 'currentTool', 'turnStartTime', 'model', 'context', 'todos', 'goal', 'taskReferenceAvailable'];
