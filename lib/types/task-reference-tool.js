    ...
    return {
        ...
        async execute(args, exec) {
            ...
            if (requested !== undefined) {
                ...
                if (idExact.length === 1)
                    anchor = { sessionId: idExact[0].sessionId, label: idExact[0].label };
                else if (titleExact.length === 1)
                    anchor = { sessionId: titleExact[0].sessionId, label: titleExact[0].label };
                else if (titleExact.length > 1)
                    return { status: 'choose', candidates: titleExact };
...
function currentPageTask(currentTask, agent) {
    ...
    if (latest !== undefined && latest.sourceKind === 'plugin')
        return undefined;
    return currentTask();
}
