...
export function createTaskReferenceAdapter(deps) {
    return {
        async prepare(input) {
            ...
            if (denied.has(trace.target.header.id) || denied.has(root.header.id))
                throw new Error('cannot reference an assistant-owned session');
            ...
            if (all.length === 0)
                throw new Error('task has no referenceable sessions');
...
function latestTime(events) {
    ...
    for (const event of events) {
        if (typeof event.time === 'number' && Number.isFinite(event.time) && (latest === undefined || event.time > latest))
            latest = event.time;
    }
    return latest;
}
//# sourceMappingURL=task-reference.js.map