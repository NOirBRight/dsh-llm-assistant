...
export async function handleAssistantRpc(
  ...
  if (endpoint === ASSISTANT_SEND_ENDPOINT) {
    ...
    if (request === undefined) return fail('bad-request', 'invalid assistant/send request: expected a non-empty text field')
  ...
  if (endpoint === ASSISTANT_IMAGE_ENDPOINT) {
    ...
    if (request === undefined) return fail('bad-request', 'invalid assistant/image request')
    const image = await port.readImage(request.attachmentId)
    if (image === undefined) return fail('not-found', 'image not found')
    return { ok: true, value: image }
  }
  if (endpoint === ASSISTANT_ROLLOVER_ENDPOINT) {
    if (!isRecord(payload) || Object.keys(payload).length !== 0) return fail('bad-request', 'invalid assistant/rollover request')
    if (extras.rollover === undefined) return fail('unavailable', 'new assistant conversation is not available')
    return extras.rollover()
  }
  if (endpoint === ASSISTANT_SET_MODEL_ENDPOINT) {
    const request = decodeSetModelRequest(payload)
    if (request === undefined) return fail('bad-request', 'invalid assistant/set-model request: expected a model id')
    if (extras.setModel === undefined) return fail('unavailable', 'model selection is not available')
