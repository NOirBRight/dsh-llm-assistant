...
export function buildSessionHandoff(snapshot: AssistantSnapshot): string {
  ...
  if (snapshot.goal !== undefined) lines.push('当前目标：' + clip(snapshot.goal.title, 800))
  ...
  const recentAssistant = [...snapshot.messages].reverse().find((message) =>
    message.role === 'assistant' && message.text.trim() !== '' && (recentUser === undefined || message.seq > recentUser.seq),
  )
  if (!structured) {
    if (recentUser !== undefined) lines.push('当前焦点：' + clip(recentUser.text, 500))
    if (recentAssistant !== undefined) lines.push('上次结论：' + clip(recentAssistant.text, 500))
  }
...
export interface RolloverSession {
  ...
  readonly header: { readonly seedLength?: number; readonly cwd?: string }
...
export interface SessionRollover {
  ...
  rollover(): Promise<{ sessionId: string }>
}

export class SessionRolloverError extends Error {
  constructor(readonly code: 'busy' | 'switching', message: string) {
    super(message)
    this.name = 'SessionRolloverError'
  }
}

export function createSessionRollover(deps: SessionRolloverDependencies): SessionRollover {
  let switching = false
  return {
    get switching(): boolean { return switching },
    async rollover(): Promise<{ sessionId: string }> {
      ...
      switching = true
      let nextHandle: RolloverAgentHandle | undefined
      let committed = false
      try {
        ...
        if (!committed && nextHandle !== undefined) {
          try { await nextHandle.dispose() } catch (disposeError) { deps.warn('新助理会话回滚失败：' + errorMessage(disposeError)) }
        }
        throw error
      } finally {
        switching = false
